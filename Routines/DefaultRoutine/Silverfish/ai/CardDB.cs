...
namespace HREngine.Bots
{
    public partial class CardDB
    {
        ...
        public class Card
        {
            ...
            public List<Race> Races = new List<Race>();//双种族
            ...
            public List<Minion> getTargetsForCard(Playfield p, bool isLethalCheck, bool own)
            {
                ...
                if ((this.type == CardDB.cardtype.MOB||this.type == CardDB.cardtype.LOCATION) && ((own && p.ownMinions.Count >= 7) || (!own && p.enemyMinions.Count >= 7))) return retval; // cant play mob, if we have allready 7 mininos
            ...
            public List<Minion> getTargetsForLocation(Playfield p, bool isLethalCheck, bool own)
            {
                ...
                else
                {
                    ...
                    foreach (Minion m in targets)
                    {
                        ...
                        {
                            if (m.stealth && !m.own) continue;
                            if (m.cantBeTargetedBySpellsOrHeroPowers && (this.type == CardDB.cardtype.SPELL || this.type == CardDB.cardtype.HEROPWR || this.type == CardDB.cardtype.LOCATION)) continue;
                            retval.Add(m);
                        }
                        m.extraParam = false;
                    }
            ...
            public int getManaCost(Playfield p, int currentcost)
            {
                ...
                switch (this.nameEN)
                {
                    ...
                    case CardDB.cardNameEN.libramofhope://希望圣契
                        retval = retval + offset - p.libram;
                        break;
                    case CardDB.cardNameEN.libramofclarity://明澈圣契
                        retval = retval + offset - p.libram;
                        break;
                    case CardDB.cardNameEN.libramofdivinity://神性圣契
                        retval = retval + offset - p.libram;
                        break;   
                    ...
                    case CardDB.cardNameEN.seagiant:
                        ...
                        break;
					case CardDB.cardNameEN.eredarbrute://艾瑞达蛮兵减费
                        retval = retval + offset - p.enemyMinions.Count + p.enemyMobsCountStarted;
                        break;
                    case CardDB.cardNameEN.prismaticbeam://棱彩光束减费
                        retval = retval + offset - (p.enemyMinions.Count+休眠随从) + (p.enemyMobsCountStarted + 休眠随从);
                        break;
                    case CardDB.cardNameEN.zilliaxdeluxe3000://奇利亚斯
                        retval = retval + offset - (p.ownMinions.Count + 休眠随从) + (p.ownMobsCountStarted + 休眠随从);
                        break;
