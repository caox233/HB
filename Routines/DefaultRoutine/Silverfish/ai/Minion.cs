...
namespace HREngine.Bots
{
    ...
    public class Minion
    {
        ...
        // 灌注机制实现
        public class InfuseSystem
        {
            public bool HasInfusedHeroPower = false; // 是否已获得灌注英雄技能
            public int InfuseCount = 0; // 灌注次数计数器
            public CardDB.cardIDEnum OriginalHeroPower; // 原始英雄技能ID
            public CardDB.cardIDEnum InfusedHeroPower; // 灌注后的英雄技能ID

            // 应用灌注效果
            public void ApplyInfuse()
            {
                if (!HasInfusedHeroPower)
                {
                    // 首次灌注，替换英雄技能
                    HasInfusedHeroPower = true;
                    // 根据职业选择对应的灌注技能
                }
                else
                {
            ...
            get
            {
                return "{" + zonepos.ToString() + "} " + " (" + Angr + "/" + Hp + ") " + handcard.card.nameCN + "\n " +
                    ...
                    + (divineshild ? "[圣盾]" : "") + (stealth ? "[隐身]" : "") + (immune ? "[免疫]" : "") + (untouchable ? "[无法被攻击]" : "") + (lifesteal ? "[吸血]" : "")
                     + (dormant != 0 ? "[休眠(" + dormant.ToString() + ")]" : "") + (reborn ? "[复生]" : "") + (zerg ? "[异虫]" : "") + (handcard.card.Elusive ? "[扰魔]" : "") + (poisonous ? "[剧毒]" : "") + (superwindfury ? "[超级风怒]" : "");
            }
        }
        ...
        public Minion(Minion m)
        {
            ...
            this.superwindfury = m.superwindfury;
        ...
        public void getDamageOrHeal(int dmg, Playfield p, bool isMinionAttack, bool dontCalcLostDmg)
        {
            ...
            if (this.isHero)
            {
                ...
                else
                {
                    ...
                    if (p.enemyWeapon.name == CardDB.cardNameEN.bulwarkofazzinoth && p.enemyWeapon.Durability > 0) dmg = 1;
                    if (p.enemyWeapon.name == CardDB.cardNameEN.bulwarkofazzinoth) p.enemyWeapon.Durability--;;
            ...
            {
                if (this.handcard.card.cardIDenum == CardDB.cardIDEnum.VAC_936 && !this.silenced)
                {
                    damage = 8 * damage;
                }
                if (p.我方背刺王)
                {
                    if (!this.own && !this.wounded)
                    {
                        damage = 2 * damage;
                    }
                }
                else if (p.敌方背刺王) // 敌方随从
                {
                    if (this.own && !this.wounded)
                    {
                        damage = 2 * damage;
        ...
        public void updateReadyness()
        {
            ...
            if (isHero)
            {
                ...
                // 计算英雄风怒攻击次数（受沉默影响）
                int heroMaxAttacks = 1;
                if (!silenced)
                {
                    if (superwindfury) heroMaxAttacks = 4;
                    else if (windfury) heroMaxAttacks = 2;
                }

                bool canAttack = !frozen
                    && Angr > 0
                    && numAttacksThisTurn < heroMaxAttacks;
            ...
            // 以下是随从逻辑（保持原有结构）
            if (cantAttack && this.handcard.card.type != CardDB.cardtype.LOCATION) return;
            if (this.handcard.card.dormant > 0 && !this.silenced && this.playedThisTurn && this.handcard.card.type != CardDB.cardtype.LOCATION) return;

            int maxAttacks = 1;
            if (!silenced)
            {
                maxAttacks = superwindfury ? 4 : windfury ? 2 : 1;
            }

            if (frozen) return;
            if (numAttacksThisTurn >= maxAttacks) return;

            if (playedThisTurn && rush >= 1)
            {
                Ready = true;
            ...
            if (this.handcard.card.type == CardDB.cardtype.LOCATION && (playedThisTurn || this.CooldownTurn == 0))
            {
                Ready = true;
            }
        ...
        public Minion GetTargetForMinionWithSurvival(Playfield p, bool own)
        {
            ...
            foreach (Minion m in own ? p.enemyMinions : p.ownMinions)
            {
                ...
                {
                    if (this.Hp > m.Hp && (m.Hp + m.Angr + m.Angr * (m.windfury ? 1 : 0) + m.Angr * (m.superwindfury ? 3 : 0)) > (targetTaumt.Hp + targetTaumt.Angr + targetTaumt.Angr * (targetTaumt.superwindfury ? 3 : 0) + targetTaumt.Angr * (targetTaumt.windfury ? 1 : 0))) targetTaumt = m;
                }
                else
                {
                    if (this.Hp > m.Hp && (m.Hp + m.Angr + m.Angr * (m.windfury ? 1 : 0)) + m.Angr * (m.superwindfury ? 3 : 0) > (target.Hp + target.Angr + target.Angr * (target.superwindfury ? 3 : 0) + target.Angr * (target.windfury ? 1 : 0))) target = m;
                }
            }
        ...
        public void loadEnchantments(List<miniEnch> enchants, int ownPlayerControler)
        {
            foreach (miniEnch me in enchants)
            {
                ...
                switch (me.CARDID)
                {
                    ...
                    case CardDB.cardIDEnum.EDR_261e: this.souloftheforest++; continue;
                    case CardDB.cardIDEnum.DMF_734e: this.stegodon++; continue;
                    ...
                    case CardDB.cardIDEnum.SC_002e: if (ownPlayerControler == Hrtprozis.Instance.getOwnController()) Hrtprozis.Instance.updateInfestor(1); continue;
