...
namespace HREngine.Bots
{
    public class Ai
    {
        ...
        public void simmulateWholeTurn(BoardTester bt)  // 这里影响输出.result文件
        {
            ...
            {
                enemyVal += m.handcard.card.nameCN + " ( " + m.Angr + " / " + m.Hp + " ) " + (m.frozen ? "[冻结]" : "") + (!m.Ready || m.cantAttack ? "[无法攻击]" : "") + (m.windfury ? "[风怒]" : "") + (m.taunt ? "[嘲讽]" : "") + (m.superwindfury ? "[超级风怒]" : "");
            }
            foreach (Minion m in p.ownMinions)
            {
                myVal += m.handcard.card.nameCN + " ( " + m.Angr + " / " + m.Hp + " ) " + (m.frozen ? "[冻结]" : "") + (!m.Ready || m.cantAttack ? "[无法攻击]" : "") + (m.windfury ? "[风怒]" : "") + (m.taunt ? "[嘲讽]" : "") + (m.zerg ? "[异虫]" : "") + (m.superwindfury ? "[超级风怒]" : "");
            }
            ...
            normalInfo = "水晶： " + tempbestboard.mana + " / " + tempbestboard.ownMaxMana
                + " [我方英雄] " + tempbestboard.ownHeroName + " （生命: " + tempbestboard.ownHero.Hp + " + " + tempbestboard.ownHero.armor
                + (tempbestboard.ownHero.divineshild ? " 圣盾" : "")
                + (tempbestboard.ownSecretsIDList.Count > 0 ? " 奥秘数: " + tempbestboard.ownSecretsIDList.Count : "")
                + (tempbestboard.getCorpseCount() > 0 ? " 尸体数: " + tempbestboard.getCorpseCount() : "")
                + (tempbestboard.ownAuraIDList.Count > 0 ? " 光环数: " + tempbestboard.ownAuraIDList.Count : "")
                + "）"  // 统一中文全角闭合

                + " [敌方英雄] " + tempbestboard.enemyHeroName + " （生命: " + tempbestboard.enemyHero.Hp + " + " + tempbestboard.enemyHero.armor
                + (tempbestboard.enemySecretCount > 0 ? " 奥秘数: " + tempbestboard.enemySecretCount : "")
                + (tempbestboard.enemyHero.divineshild ? " 圣盾" : "")
                + (tempbestboard.enemyHero.immune ? " 免疫" : "")
                + "）"  // 统一中文全角闭合
            ...
            {
                enemyVal += m.handcard.card.nameCN + " ( " + m.Angr + " / " + m.Hp + " ) " + (m.frozen ? "[冻结]" : "") + (!m.Ready || m.cantAttack ? "[无法攻击]" : "") + (m.reborn ? "[复生]" : "") + (m.zerg ? "[异虫]" : "") + (m.divineshild ? "[圣盾]" : "") + (m.windfury ? "[风怒]" : "") + (m.taunt ? "[嘲讽]" : "") + (m.superwindfury ? "[超级风怒]" : "");
            }
            foreach (Minion m in tempbestboard.ownMinions)
            {
                myVal += m.handcard.card.nameCN + " ( " + m.Angr + " / " + m.Hp + " ) " + (m.frozen ? "[冻结]" : "") + (!m.Ready || m.cantAttack ? "[无法攻击]" : "") + (m.reborn ? "[复生]" : "") + (m.zerg ? "[异虫]" : "") + (m.divineshild ? "[圣盾]" : "") + (m.windfury ? "[风怒]" : "") + (m.taunt ? "[嘲讽]" : "") + (m.superwindfury ? "[超级风怒]" : "");
            }
