namespace HREngine.Bots
{
    ...
    public class Movegenerator
    {
        ...
        public List<Action> getMoveList(Playfield p, bool usePenalityManager, bool useCutingTargets, bool own)
        {
            ...
            if (ret == null) ret = new List<Action>();
            if (p.complete || p.ownHero.Hp <= 0) return ret;

            List<Minion> trgts = null;
            if (trgts == null) trgts = new List<Minion>();

            if (own)
            {
                HashSet<string> playedcards = null;
                if (playedcards == null) playedcards = new HashSet<string>();
                StringBuilder cardNcost = null;
                if (cardNcost == null) cardNcost = new StringBuilder();
            ...
            // 处理随从攻击
            List<Minion> attackingMinions = null;
            if (attackingMinions == null) attackingMinions = new List<Minion>();
            foreach (var m in p.ownMinions)
            {
                if (m.numAttacksThisTurn == 1 && !m.frozen && !m.cantAttack)
                {
                    ...
                    if (m.superwindfury && !m.silenced && !m.frozen)
                    {
                        m.Ready = true;
                    }
                }

                if (m.numAttacksThisTurn == 2 && !m.frozen && !m.cantAttack)
                {
                    m.Ready = m.superwindfury && !m.silenced && !m.frozen;
                }

                if (m.numAttacksThisTurn == 3 && !m.frozen && !m.cantAttack)
                {
                    m.Ready = m.superwindfury && !m.silenced && !m.frozen;
                }
            ...
            // 使用地标逻辑
            List<Minion> usingMinions = null;
            if (usingMinions == null) usingMinions = new List<Minion>();
            usingMinions = (own ? p.ownMinions : p.enemyMinions)
            ...
            // 使用星舰逻辑
            List<Minion> starships = null;
            if (starships == null) starships = new List<Minion>();
            starships = (own ? p.ownMinions : p.enemyMinions)
                .Where(m => m.handcard.card.type == CardDB.cardtype.STARSHIP && m.Ready)
                .ToList();
            foreach (var minion in starships)
            {
                trgts.Clear();
                trgts = minion.handcard.card.getTargetsForStarship(p, p.isLethalCheck, true);
                if (trgts.Count > 0)
                {
                    foreach (var trot in trgts)
                    {
                        int useStarshipPenalty = usePenalityManager ? pen.getUseStarshipPenality(minion, trot, p) : 0;
                        if (useStarshipPenalty <= 499)
                        {
                            ret.Add(new Action(actionEnum.useStarship, null, minion, 0, trot, 0, 0));
                        }
                    }
                }
                else if (minion.handcard.card.sim_card.GetUseAbilityReqs().Length == 0)
                {
                    ret.Add(new Action(actionEnum.useStarship, null, minion, 0, null, 0, 0));
                }
            }

            // 使用泰坦技能逻辑
            List<Minion> titans = null;
            if (titans == null) titans = new List<Minion>();
            titans = (own ? p.ownMinions : p.enemyMinions).Where(m => m.handcard.card.Titan).ToList();
        ...
        {
            List<Minion> retvalues = null;
            if (retvalues == null) retvalues = new List<Minion>(oldlist.Count);
            List<Minion> addedmins = null;
            if (addedmins == null) addedmins = new List<Minion>(oldlist.Count);
            ...
            foreach (Minion m in oldlist)
            {
                ...
                foreach (Minion mnn in addedmins)
                {
                    ...
                    if ((onlySpecial || onlyNotSpecial) && (mnn.Angr == m.Angr && mnn.Hp == m.Hp && mnn.divineshild == m.divineshild && mnn.reborn == m.reborn && mnn.taunt == m.taunt && mnn.zerg == m.zerg && mnn.poisonous == m.poisonous && mnn.lifesteal == m.lifesteal && m.handcard.card.isToken == mnn.handcard.card.isToken && mnn.handcard.card.race == m.handcard.card.race && mnn.Spellburst == m.Spellburst && mnn.cantAttackHeroes == m.cantAttackHeroes))
                    {
                        goingtoadd = false;
                        break;
                    }
