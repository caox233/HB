...
namespace HREngine.Bots
{
    ...
    public partial class Behavior丨标准丨光牙DK : Behavior
    {
        ...
        {
            if (!m.silenced && m.handcard.card.CantAttack ||
                target.untouchable)
                return 1000;
            // 保留，别送
            int pen = -10;
            if (m.handcard.card.nameCN == CardDB.cardNameCN.脆弱的食尸鬼 && target.isHero) pen += 5;
        ...
        private int getSecretPenality(Playfield p)  // 牌序和防奥秘的影响
        {
            ...
            for (int i = 0; i < p.playactions.Count; i++)
            {
                ...
                if (a.actionType == actionEnum.playcard)
                {
                    ...
                    if (a.card.card.nameCN == CardDB.cardNameCN.烈焰亡魂) flag |= 2;
                }
            }
