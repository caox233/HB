using System;
using System.Collections.Generic;
using System.Text;

namespace HREngine.Bots
{
	//随从 圣骑士 费用：2 攻击力：3 生命值：2
	//Blood Matriarch Liadrin
	//血骑士领袖莉亚德琳
	//After you summon a minion with less Attack than this, give it <b>Divine Shield</b> and <b>Rush</b>.
	//在你召唤攻击力低于本随从的随从后，使其获得<b>圣盾</b>和<b>突袭</b>。
	class Sim_RLK_924 : SimTemplate
	{
        public override void onMinionWasSummoned(Playfield p, Minion own, Minion summonedMinion)
        {
            int ang = own.Angr;
            if (summonedMinion.Angr < ang && own.own == summonedMinion.own && own.entitiyID != summonedMinion.entitiyID)
            {
                summonedMinion.divineshild = true;
                p.minionGetRush(summonedMinion);
            }
        }
    }
}
