...
namespace HREngine.Bots
{
    public class Silverfish
    {
        ...
        private List<string> ownAuraList = new List<string>();
        ...
        private int 本局获得尸体数 = 0;
        private int 本局消耗尸体数 = 0;
        ...
        private void updateRealTimeInfo()
        {
            ...
            本局获得尸体数 = TritonHs.Corpses;
            本局消耗尸体数 = TritonHs.CORPSES_SPENT_THIS_GAME;
        ...
        private void updateSecret(HSCard card, Entity entity, int controller, string cardId, int entityId)
        {
            ...
            // 支线任务
            else if (entity.GetTag(1192) > 0 || entity.GetTag(2311) > 0)
            {
                if (controller == ownController)
                {
                    int currentQuestProgress = entity.GetTag(GAME_TAG.QUEST_PROGRESS);
        ...
        private void updateHero(HSCard card, Entity entity, int controller, int entityId, string cardId)
        {
            if (controller == ownController)
            {
                ...
                this.ownHero.脱罪 = card.GetTag(GAME_TAG.HEAVILY_ARMORED) != 0;
				this.ownHero.divineshild = card.GetTag(GAME_TAG.DIVINE_SHIELD) != 0;
            ...
            else if (controller == enemyController)
            {
                ...
                this.enemyHero.脱罪 = card.GetTag(GAME_TAG.HEAVILY_ARMORED) != 0;
                this.enemyHero.divineshild = card.GetTag(GAME_TAG.DIVINE_SHIELD) != 0;
        ...
        private void updateMinion(HSCard card, Entity entity, int controller, string cardId)//随从效果
        {
            ...
            if (zp >= 1)
            {
                ...
                m.CooldownTurn = card.GetTag(GAME_TAG.LOCATION_ACTION_COOLDOWN);//地标冷却回合？
                ...
                m.superwindfury = (card.GetTag(GAME_TAG.MEGA_WINDFURY) == 0) ? false:true;//超级风怒
                ...
                m.reborn = (card.GetTag(GAME_TAG.REBORN) == 0) ? false : true;//复生

                m.zerg = (card.GetTag(GAME_TAG.ZERG) == 0) ? false : true;//异虫
                ...
                if(m.nameCN == CardDB.cardNameCN.步移山丘){ //超级风怒
                    m.superwindfury = true;
                }
                ...
                Actor actor = card.Card.GetActor();
                if (actor != null)
                {
                    // Log.ErrorFormat("actor.GetActorStateType()={0}", actor.GetActorStateType());
                }
                // 计算最大攻击次数（需从类成员提取或作为参数传入）
                int maxAttacks = !m.silenced ?
                    (m.superwindfury ? 4 : m.windfury ? 2 : 1) : 1;

                if (m.rush > 0 && m.numAttacksThisTurn < maxAttacks)
                {
                    m.Ready = !m.frozen && !m.cantAttack;
                    m.cantAttackHeroes = m.playedThisTurn; // 当回合突袭限制

                    // 冻结状态最终判断
                    if (m.frozen) m.Ready = false;
                }
        ...
        private void printstuff()  //会输出到文件夹UltimateLogs里面
        {
            ...
            + (p.ownAuraIDList.Count > 0 ? " 光环数: " + p.ownAuraIDList.Count : "")
            + (p.getCorpseCount() > 0 ? " 尸体数: " + p.getCorpseCount() : "")
            + "） " // 统一中文括号闭合
            ...
            + (p.enemySecretCount > 0 ? " 奥秘数: " + p.enemySecretCount : "")
            + (p.enemyHero.immune ? " 免疫" : "")
            + (p.enemyHero.divineshild ? " 圣盾" : "")
            + "） "; // 统一中文括号闭合
            foreach (Minion m in p.enemyMinions)
            {
                enemyVal += m.handcard.card.nameCN + " ( " + m.Angr + " / " + m.Hp + " )" + (m.frozen ? "[冻结]" : "") + (m.cantAttack ? "[无法攻击]" : "") + (m.windfury ? "[风怒]" : "") + (m.superwindfury ? "[超级风怒]" : "") + (m.taunt ? "[嘲讽]" : " ");
            }
            foreach (Minion m in p.ownMinions)
            {
                myVal += m.handcard.card.nameCN + " ( " + m.Angr + " / " + m.Hp + " )" + (m.frozen ? "[冻结]" : "") + (!m.Ready || m.cantAttack ? "[无法攻击]" : "") + (m.windfury ? "[风怒]" : "") + (m.superwindfury ? "[超级风怒]" : "") + (m.zerg ? "[异虫]" : "") + (m.taunt ? "[嘲讽]" : " ");
            }
            ...
            Helpfunctions.Instance.logg("own auraCount: " + ownAuraList.Count);
